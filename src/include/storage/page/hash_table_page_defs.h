 ...
 */
#define BLOCK_ARRAY_SIZE (4 * static_cast<size_t>(PAGE_SIZE) / (4 * sizeof(MappingType) + 1))

/**
 * Extendable Hashing Definitions
 */
#define HASH_TABLE_BUCKET_TYPE HashTableBucketPage<KeyType, ValueType, KeyComparator>

/**
 * BUCKET_ARRAY_SIZE is the number of (key, value) pairs that can be stored in an extendable hashing bucket page.
 ...
 * to maintain the occupied and readable flags for a key value pair.
 */
#define BUCKET_ARRAY_SIZE (4 * static_cast<size_t>(PAGE_SIZE) / (4 * sizeof(MappingType) + 1))
